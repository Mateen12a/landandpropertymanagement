...
(function (modules, entry, mainEntry, parcelRequireName, globalName) {
 ...
 if (loginForm) loginForm.addEventListener("submit", (e)=>{
  ...
  if(searchForm) searchForm.addEventListener("submit", async function (e){
      e.preventDefault();
    
      const query = document.querySelector('input[name="query"]').value;
      const response = await fetch(`/api/v1/property/search?query=${encodeURIComponent(query)}`);
      ...
      if (data.results > 0) {
        ...
        window.location.href = `/properties/search?query=${encodeURIComponent(query)}`;
      } else {
...
if (unverifyUser) {
  ...
  deleteButtons.forEach(button => {
    button.addEventListener('click', async (event) => {
      ...
      if (confirm('Are you sure you want to delete this property?')) {
        (0, _handleFormSubmit.deleteProperty)(id)
      }
    });
  });
  ...
  if (propertyForm) propertyForm.addEventListener("submit", (e)=>{
      ...
      (0, _handleFormSubmit.addProperty)(form);
  });
  const getPropertyIdFromUrl = () => {
    const pathArray = window.location.pathname.split('/');
    return pathArray.find(segment => segment.length === 24); // Assuming MongoDB ObjectID length
  };
  if (propertyFormUpdate) propertyFormUpdate.addEventListener("submit", (e)=>{
    ...
    (0, _handleFormSubmit.updateProperty)(form, propertyId);
  });
  ...
  const addProperty = async (data) => {
    try {
      if (document.querySelector(".type").value.trim().toLowerCase() !== "land") {
              const amenities = [
          {
            amenity: (+document.querySelector(".quantity-bed").value.trim() > 1) ? "beds" : "bed",
            quantity: +document.querySelector(".quantity-bed").value.trim()
          },
          {
            amenity: (+document.querySelector(".quantity-bath").value.trim() > 1) ? "baths" : "bath",
            quantity: +document.querySelector(".quantity-bath").value.trim()
          },
          {
            amenity: (+document.querySelector(".quantity-toilet").value.trim() > 1) ? "toilets" : "toilet",
...
const updateProperty = async (data, id) => {
    try {
      ...
      if (res.data.status === "success") {
        (0, _alert.showAlert)("success", "Property Updated Successfully");
        setTimeout(() => {
          window.location.assign(`/property/${res.data.data.data._id}`);
        }, 1500);
      }
